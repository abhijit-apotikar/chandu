...
//check Availability******************
  ...
  Future checkSemForElectives(String cName, String bName, String sem) async {
    QuerySnapshot qs = await fireStoreInstance
        .collection('courses')
        .where('cName', isEqualTo: cName)
        .get();
    for (int i = 0; i < qs.docs[0].data()['cScheme']['branches'].length; i++) {
      if (qs.docs[0].data()['cScheme']['branches'][i]['bName'] == bName) {
        if (qs.docs[0]
            .data()['cScheme']['branches'][i]['aSems']
            .contains(sem)) {
          if (qs.docs[0].data()['cScheme']['branches'][i]['aSems']
                  ['hasElectives'] ==
              true) {
            return true;
          } else {
        ...
        for (int j = 0;
          ...
          if (qs.docs[0].data()['cScheme']['branches'][i]['aSems'][j]
            ...
            for (int k = 0;
                k <
                    qs.docs[0]
                        .data()['cScheme']['branches'][i]['aSems']['subjects']
                        .length;
                k++) {
